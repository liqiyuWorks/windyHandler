...
try:
    ...
    from pkg.tools import cached_property
...
class Function:
    ...
    def set_outputs(self, outputs):
        ...
        self : Function
        """
        self.__outputs__ = [outputs] if isinstance(outputs,
                                                   str) else list(outputs)
    ...
    def set_source(self, source):
        ...
        # Handle ndarray source
            ...
            dV = self.__inputs__ == ["Scalar"
                                     ] and self.__outputs__ == ["Scalar"]
            ...
            if self.__dom_dim__ == 1:
                ...
                self.y_initial, self.y_final = self.y_array[0], self.y_array[
                    -1]
            ...
            # Do things if function is multivariate
                ...
                self.z_initial, self.z_final = self.z_array[0], self.z_array[
                    -1]
    ...
    def set_discrete(
        ...
        if self.__dom_dim__ == 1:
            ...
            ys = self.get_value(
                xs.tolist()) if one_by_one else self.get_value(xs)
            ...
            sam = 2 * [samples] if isinstance(samples,
                                              (int, float)) else samples
    ...
    def set_discrete_based_on_model(self,
                                    model_function,
                                    one_by_one=True,
                                    keep_self=True):
        ...
        if model_function.__dom_dim__ != self.__dom_dim__:
            raise ValueError(
                "model_function must have the same domain dimension.")
        ...
        interp = (self.__interpolation__
                  if keep_self else model_function.__interpolation__)
        extrap = (self.__extrapolation__
                  if keep_self else model_function.__extrapolation__)
        ...
        # Returns value for spline, akima or linear interpolation function type
            ...
            if self.__interpolation__ == "spline":
                ...
                for i in range(len(x)):
                    ...
                    elif x_min < x[i] < x_max or (self.__extrapolation__
                                                  == "natural"):
                        if not x_min < x[i] < x_max:
                            a = coeffs[:, 0] if x[i] < x_min else coeffs[:, -1]
                            x[i] = (x[i] -
                                    x_data[0] if x[i] < x_min else x[i] -
                                    x_data[-2])
                        else:
                            a = coeffs[:, x_intervals[i] - 1]
                            x[i] = x[i] - x_data[x_intervals[i] - 1]
                        x[i] = a[3] * x[i]**3 + a[2] * x[i]**2 + a[1] * x[
                            i] + a[0]
                    else:
                        # Extrapolate
                        if self.__extrapolation__ == "zero":
                ...
                for i in range(len(x)):
                    ...
                    if x_min <= x[i] <= x_max:
                        ...
                        if self.__extrapolation__ == "zero":  # Extrapolation == zero
                            ...
                            dx = float(x_data[inter] - x_data[inter - 1])
                            dy = float(y_data[inter] - y_data[inter - 1])
                            x[i] = (x[i] - x_data[inter - 1]) * (
                                dy / dx) + y_data[inter - 1]
                        else:  # Extrapolation is set to constant
                            x[i] = y_data[0] if x[i] < x_min else y_data[-1]
            else:
    ...
    def to_frequency_domain(self,
        ...
        fourier_amplitude = np.abs(
            np.fft.fft(sampled_points) / (number_of_samples / 2))
        fourier_frequencies = np.fft.fftfreq(number_of_samples,
                                             sampling_time_step)
        return Function(
    ...
    def __str__(self):
        ...
        return str("Function from R" + str(self.__dom_dim__) + " to R" +
    ...
    def __repr__(self):
        "Return a string representation of the Function"
        return repr("Function from R" + str(self.__dom_dim__) + " to R" +
                    str(self.__img_dim__) + " : (" +
                    ", ".join(self.__inputs__) + ") → (" +
                    ", ".join(self.__outputs__) + ")")
    ...
    def set_title(self, title):
        if title:
            ...
            if self.__dom_dim__ == 1:
                self.title = (self.__outputs__[0].title() + " x " +
    ...
    def plot(self, *args, **kwargs):
        ...
        if isinstance(self, list):
            ...
            if self.__dom_dim__ == 1:
                ...
                print(
                    "Error: Only functions with 1D or 2D domains are plottable!"
                )
    ...
    def plot1D(
        ...
        if callable(self.source):
            ...
            up_ind = len(x_data) - 1 if too_high else np.where(
                x_data <= upper)[0][0]
    ...
    def plot2D(
        ...
        mesh = [[mesh_x_flat[i], mesh_y_flat[i]]
                for i in range(len(mesh_x_flat))]
        # Evaluate function at all mesh nodes and convert it to matrix
    ...
    def __interpolate_spline__(self):
        ...
        for i in range(1, mdim - 1):
            ...
            B.append(3 * ((y[i + 1] - y[i]) / (h[i]) - (y[i] - y[i - 1]) /
                          (h[i - 1])))
    ...
    def __add__(self, other):
        ...
        try:
            ...
            if (isinstance(other.source, np.ndarray)
        ...
        if self.__interpolation__ == "spline" and numerical is False:
            ...
            if a < x_data[0]:
                if self.__extrapolation__ == "constant":
                    ...
                    ans += ((c[3] * sub_a**4) / 4 + (c[2] * sub_a**3 / 3) +
                            (c[1] * sub_a**2 / 2) + c[0] * sub_a)
                    ans -= ((c[3] * sub_b**4) / 4 + (c[2] * sub_b**3 / 3) +
                            (c[1] * sub_b**2 / 2) + c[0] * sub_b)
                else:
            ...
            while i < len(x_data) - 1 and x_data[i] < b:
                if x_data[i] <= a <= x_data[
                        i + 1] and x_data[i] <= b <= x_data[i + 1]:
                    sub_a = a - x_data[i]
                    sub_b = b - x_data[i]
            ...
            if self.__extrapolation__ == "zero":
                if a >= x_data[0]:
                    x_integration_data = np.concatenate(
                        ([a], x_integration_data))
                    y_integration_data = np.concatenate(
                        ([self(a)], y_integration_data))
                if b <= x_data[-1]:
                    x_integration_data = np.concatenate(
                        (x_integration_data, [b]))
                    y_integration_data = np.concatenate(
                        (y_integration_data, [self(b)]))
            else:
    ...
    def differentiate(self, x, dx=1e-6, order=1):
        ...
        if order == 1:
            ...
            return (self.get_value(x + dx) - 2 * self.get_value(x) +
                    self.get_value(x - dx)) / dx**2
    ...
    def isbijective(self):
        ...
        if isinstance(self.source, np.ndarray):
            ...
            return len(distinct_map) == len(x_data_distinct) == len(
                y_data_distinct)
        else:
            raise TypeError(
    ...
    def inverse_function(self, approx_func=None, tol=1e-4):
        ...
        if isinstance(self.source, np.ndarray):
            ...
            if approx_func is not None:
                source = lambda x: self.find_input(
                    x, start=approx_func(x), tol=tol)
            else:
    ...
    def compose(self, func, extrapolate=False):
        ...
        if isinstance(self.source, np.ndarray) and isinstance(
                func.source, np.ndarray):
