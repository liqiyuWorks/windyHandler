...
def geopotential_to_height_agl(geopotential,
                               elevation,
                               radius=63781370,
                               g=9.80665):
...
def find_closest(ordered_sequence, value):
    ...
    smaller, greater = ordered_sequence[pivot_index -
                                        1], ordered_sequence[pivot_index]
...
def check_requirement_version(module_name, version):
    ...
    operator, v_number = re.match(f"({'|'.join(operators)})(.*)",
                                  version).groups()
    ...
    installed_version = packaging_version.parse(
